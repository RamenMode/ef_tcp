 ...
 */
#define RX_DMA_OFF           ROUND_UP(sizeof(struct pkt_buf), EF_VI_DMA_ALIGN) // 64

#define RX_RING_SIZE         512
#define TX_RING_SIZE         2048


struct pkt_buf { // this is a physical buffer, not virtual buffer
...
#define REFILL_BATCH_SIZE  64
  ...
  if( ef_vi_receive_space(vi) < REFILL_BATCH_SIZE || // if the RXQ is pretty full or there's not enough resources left
...
static int init(const char* intf, int vi_i)
{
    ...
    TRY(ef_vi_alloc_from_pd(&vi->vi, vi->dh, &vi->pd, vi->dh, -1, RX_RING_SIZE, TX_RING_SIZE, NULL, -1, (enum ef_vi_flags) vi_flags));
